...
- **Log file** — every action is logged to `Documents\PerfTweaks_Backups\PerfTweaks_<random>.log` as a clean `[timestamp] EXEC / OK / FAIL` timeline. The log records each command's *outcome*, not its raw output, so it stays readable and doesn't fill up with deleted-file paths or (on non-English Windows) garbled OEM-code-page error text.
...
- **Idempotent registry writes — a re-apply no longer poisons the undo.** Applying a registry value that is already at the target now prints `[SKIP] … already set` and returns **before** writing a backup. Previously a re-run — or re-applying a preset — snapshotted the *already-tweaked* value as its "prior" state, quietly burying that value's true-original `.reg`/JSON undo; because the single-value restore list shows newest first, the poisoned copy was the natural pick. Re-applies (deliberate or via a preset) are now safe for both the single-value and preset-JSON undo data.
- **Safe defaults on the confirmation prompts.** The "Apply … ? (Y/N)" gate on each action, and the secondary "also do …?" confirms, now clear their variable before asking — so a bare **Enter** means *no / skip* instead of silently repeating a stale `Y` carried over from an earlier action. The **"Create a restore point?"** prompts default the other way (Enter = **Yes**). The one that mattered most: the optional **clear-all-Event-Viewer-logs** step (irreversible) can no longer fire from a stray Enter still holding an old `Y`.
- **Cleaner log file.** `:Run` no longer pipes each command's raw output into the log — it records only the `[timestamp] EXEC / OK / FAIL` lines. Before, a verbose command such as the temp-folder `del` dumped hundreds of file paths and, on non-English Windows, its OEM-code-page "file is in use" errors, which showed up as garbled symbols when the log was opened in a UTF-8 editor. Every command's outcome is still recorded (via `OK`/`FAIL`); only the noisy, mis-encodable raw output is gone.
- **SteamLight reports honestly.** The `[OK]` after installing SteamLight is now gated on the launcher `.bat` actually being written into the Steam folder, so an unwritable folder (e.g. Steam under `Program Files` without elevation) yields a clear `[ERROR]` telling you to run as administrator — instead of a false success.
